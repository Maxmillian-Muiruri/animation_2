...
confettiGeom.setAttribute(
  "position",
  new THREE.Float32BufferAttribute(confettiPos, 3),
);
confettiGeom.setAttribute(
  "color",
  new THREE.Float32BufferAttribute(confettiColors, 3),
);
...
setTimeout(() => {
  ...
  tl.to(
    confettiMat,
    { opacity: 0.9, duration: hd * 0.25, ease: "power2.out" },
    hd * 0.85,
  );
  tl.to(
    confetti.scale,
    { x: 2.5, y: 2.5, z: 2.5, duration: hd * 0.5, ease: "power2.out" },
    hd * 0.85,
  );
  tl.to(
    confettiMat,
    { opacity: 0, duration: hd * 0.3, ease: "power2.in" },
    hd * 1.35,
  );
...
for (let i = 0; i < floatingHeartCount; i++) {
  const tex = [heartTex, goldHeartTex, whiteHeartTex][
...
setTimeout(() => {
  ...
  document.querySelectorAll(".line1 span").forEach((span, i) => {
    tl.to(
      span,
      { opacity: 0, duration: (hd * 0.2) / l1, ease: "power1.in" },
      hd * 0.65 + i * ((hd * 0.25) / l1),
    );
  });
  ...
  document.querySelectorAll(".line2 span").forEach((span, i) => {
    tl.to(
      span,
      { opacity: 0, duration: (hd * 0.2) / l2, ease: "power1.in" },
      hd * 1.3 + i * ((hd * 0.1) / l2),
    );
  });
  ...
  document.querySelectorAll(".line4 span").forEach((span, i) => {
    tl.to(
      span,
      { opacity: 0, duration: (hd * 0.2) / l4, ease: "power1.in" },
      td - hd * 0.3 + i * ((hd * 0.1) / l4),
    );
  });
}, 0);
...
sparkleGeometry.setAttribute(
  "position",
  new THREE.Float32BufferAttribute(sparkleVertices, 3),
);
sparkleGeometry.setAttribute(
  "alpha",
  new THREE.Float32BufferAttribute(sparkleOpacities, 1),
);
...
setTimeout(() => {
  ...
  for (let i = 0; i < sparkleCount; i++) {
    tl.to(
      sparkleGeometry.attributes.alpha.array,
      {
        [i]: 1,
        duration: hd * 0.5,
        ease: "power1.in",
...
function render() {
  ...
  floatingHearts.forEach((sprite, i) => {
    ...
    sprite.position.x =
      ud.basePos.x + Math.sin(t * ud.speed + ud.floatOffset) * 20;
    sprite.position.y =
      ud.basePos.y + Math.cos(t * ud.speed * 0.7 + ud.floatOffset) * 15;
  ...
  petals.forEach((sprite) => {
    ...
    sprite.position.x =
      ud.baseX + Math.sin(t * ud.swaySpeed + ud.phase) * ud.swayAmp;
